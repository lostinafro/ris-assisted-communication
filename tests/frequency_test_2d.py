...
if __name__ == '__main__':
    ...
    # Cell
    rad = 50   # radius of the cell
    rad_min = np.ceil(NUM_EL_X ** 2  / 2 * c / CARRIER_FREQ)   # ff distance
    # Integrations values
    dphi = 1
    drho = 5
    azimuths = int(np.ceil(180 / dphi))
    radii = int(np.ceil((rad - rad_min) / drho))
    # UE positioning
    users = azimuths * radii
    angles = np.linspace(0, 180, azimuths)
    ue_dist = np.linspace(rad_min, rad, radii)
    # ue pos in cylindrical
    ue_pos = np.vstack((ue_dist.repeat(len(angles)), np.deg2rad(np.tile(angles, len(ue_dist))), np.zeros(users))).T

    # Varying bs positioning
    bs_angles = np.arange(45, 180, 45)
    for bs_angle in bs_angles:
        bs_pos = np.array([[rad, np.deg2rad(bs_angle), 0]])
        # Build environment
        ...
        if bs_angle == bs_angles[0]:
            ...
            cmn.printplot(af_fig, af_ax, render, af_file, OUTPUT_DIR, title=f'Array factor using standard $\phi_c$', labels=[r'$\phi_k$', r'$|A|^2$'])
            # Plotting h_ris for standard configurations
            h_ris_file = f'ris_channel'
            cmn.printplot(h_ris_fig, h_ris_ax, render, h_ris_file, OUTPUT_DIR, title=f'RIS channel using standard $\phi_c$', labels=[r'$\phi_k$', r'$|g_k|^2$', r'$\angle{g_k}$'])

        # Plotting h_dir
        h_dir_file = f'dir_channel' + filename_suffix
        title = f'direct channel using standard $\phi_c$' + title_suffix
        cmn.printplot(h_dir_fig, h_dir_ax, render, h_dir_file, OUTPUT_DIR, title=title, labels=[r'$\phi_k$', r'$|h_k|^2$', r'$\angle{h_k}$'])
        ...
        cmn.printplot(h_fig, h_ax, render, h_file, OUTPUT_DIR, title=title, labels=[r'$\phi_k$', r'$|h_k + g_k|^2$', r'$\angle{h_k + g_k}$'])
